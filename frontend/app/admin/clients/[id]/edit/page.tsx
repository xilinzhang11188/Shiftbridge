...
export default function EditClientPage() {
  ...
  const loadData = async () => {
    try {
      ...
      console.log('Sites in client data:', clientData.sites);
      console.log('Sites length:', clientData.sites?.length);
      
      setClient(clientData);
      setServices(servicesData);
      setSites(clientData.sites || []);
      
      console.log('Sites state set to:', clientData.sites || []);
      ...
      setFormData({
        ...
        requested_services: clientData.requested_services,
      });
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await clientsApi.update(parseInt(clientId), formData);
      ...
      if (err.message) {
        ...
        setError('Failed to update client. Please check all fields and try again.');
      }
    } finally {
      setSaving(false);
    }
  ...
  const handleAddSite = async () => {
    ...
    try {
      const newSite = await clientsApi.createSite(parseInt(clientId), {
      ...
      const errorMessage = err.message || (typeof err === 'string' ? err : 'Failed to add site');
      alert(errorMessage);
    }
  };
  ...
  const handleDeleteSite = async (siteId: number) => {
    ...
    try {
      await clientsApi.deleteSite(siteId);
              ...
              <div className="space-y-4">
                ...
                {sites.length > 0 && (
                  <div className="space-y-2">
                    {sites.map((site) => (
                      <div key={site.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
                        <span className="text-sm text-gray-900">{site.address}</span>
                        <button
                          type="button"
                ...
                {sites.length === 0 && (
                  <p className="text-sm text-gray-500">No sites added yet. Add a site to enable shift creation for this client.</p>
                )}
              </div>
            </div>
